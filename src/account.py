...
class Account:
    ...
    def __init__(self, seed=None):
        if seed != None:
            self.issuer_wallet = Wallet(seed=seed)
        else:
            self.issuer_wallet = generate_faucet_wallet(client=self.client)
    ...
    def issue_ticket(self, eventID: int):
        ...
        mint_tx_signed = safe_sign_and_autofill_transaction(transaction=mint_tx, wallet=self.issuer_wallet, client=self.client)
    ...
    def get_account_nfts(self):
        ...
        get_account_nfts = self.client.request(
            AccountNFTs(account=self.issuer_address)
        )
